...
def main():
    ...
    parser.add_argument("--sim", type=int, required=False, default=1, help="plt.show() at the end of the script")
    ...
    swarm = Crazyswarm(swarm_path_yaml, args=['--sim'])
    ...
    logger = simulate_trajectory(swarm, quad, quad_opt, quad_nominal, x0, x_trajectory, simulation_length, Nopt, simulation_dt, logger)
    logger.save_log()
    


def simulate_trajectory(swarm, quad, quad_opt, quad_nominal, x0, x_trajectory, simulation_length, Nopt, simulation_dt, logger):
    ...
    for i in tqdm(range(Nopt)):
        ...
        pos = cf.position()
        x[:3] = np.array([pos[0], pos[1], pos[2]])
        ...
        x_opt = x_opt_acados[1,:].ravel() # optimal state trajectory
        ...
        yaw = np.arctan2(2.0*(x_opt[5]*x_opt[6] + x_opt[3]*x_opt[4]), x_opt[3]*x_opt[3] - x_opt[4]*x_opt[4] - x_opt[5]*x_opt[5] + x_opt[6]*x_opt[6])
        cf.cmdPosition(x_opt[:3],yaw=yaw)
        timeHelper.sleep(quad_opt.optimization_dt)
        #cf.cmdFullState(x_opt[:3], x_opt[7:10], acc, yaw, omega)
